...
class TestCompanyAPI:
    ...
    def test_company_list_authenticated(self, authenticated_api_client):
    ...
    def test_company_list_only_active(self, authenticated_api_client):
    ...
    def test_company_create_authenticated(self, authenticated_api_client, agent):
    ...
    def test_company_retrieve_authenticated(self, authenticated_api_client):
    ...
    def test_company_update_by_creator(self, authenticated_api_client, agent):
    ...
    def test_company_update_by_non_creator(self, authenticated_api_client, another_agent):
    ...
    def test_company_delete_by_creator(self, authenticated_api_client, agent):
    ...
    def test_company_my_companies_endpoint(self, authenticated_api_client, agent):
    ...
    def test_company_search_by_name(self, authenticated_api_client):
    ...
    def test_company_search_by_type(self, authenticated_api_client):
        """Тест поиска компаний по типу"""
        CompanyFactory(company_type='production')
        CompanyFactory(company_type='distribution')
        
        url = reverse('company-search')
        response = authenticated_api_client.get(url, {'type': 'production'})
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == 1
        assert response.data[0]['company_type'] == 'production'
    
    def test_company_search_combined(self, authenticated_api_client):
        ...
        url = reverse('company-search')
        response = authenticated_api_client.get(url, {'q': 'Test', 'type': 'production'})
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == 1
        assert response.data[0]['name'] == 'Test Production'
    
    def test_company_serializer_fields(self, authenticated_api_client):
    ...
    def test_company_list_serializer_fields(self, authenticated_api_client):
